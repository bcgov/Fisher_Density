...
zSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    pd1.p.nodes <- model$expandNodeNames("pd1.p")
    pd1.c.nodes <- model$expandNodeNames("pd1.c")
      ...
      if(updown==0){#subtract
        ...
        if(!reject){
          ...
          if(accept) {
            ...
            mvSaved["pd1.p",1][pick,] <<- model[["pd1.p"]][pick,]
            mvSaved["pd1.c",1][pick,] <<- model[["pd1.c"]][pick,]
            ...
            model[["pd1.p"]][pick,] <<- mvSaved["pd1.p",1][pick,]
            model[["pd1.c"]][pick,] <<- mvSaved["pd1.c",1][pick,]
